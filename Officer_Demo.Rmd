...
The below code chunk is a formatting chunk, the `yInfl` sets the y axis to be 1.25 times larger than the max y value, allowing headroom for data call-outs on chart bars.   
...
**FIRST** If you're starting this demo and the `Demo_Report.docx` file exists inside the project folder: open the file, take a look if you want, then close and delete it. This file will be created as you do the demo.  

This is the chunk where the template Word file is read in. For my uses in this demo, I will use the standard templates that the officer package author David Gohel uses in his github (https://github.com/davidgohel/officer), but with some modifications of my own to the styles by choosing a different theme, adding a title and TOC (table of contents) page, and adding a logo to the Word document.   


```{r Import Report template}


# Report will be the Word document

# This object will contain the blank template file which we will add to by creating an object called ReportNew 
...
Let's import some practice data. The Facebook data set has 490 rows and 19 columns. We will remove all rows with missing data. It can be found here: https://www.kaggle.com/roxannac/facebook-data  (I will delete some things and simplify it for our use.)  
...
# define "month" (numeric month is column 4 in the facebook data set) to be strings
...
# define "weekday" (numeric weekday is column 5 in the facebook data set) as strings
weekday=c()
...
Now that we have our data in place and ready to report on, we can begin adding pages to our document. The officer package works by adding elements to a copy of the Word template file. These elements can be text, page breaks, bullet lists, tables, etc. Let's start by exploring Total.Interaction by month.  
...
# Create bar charts to use in word (tall)
...
# Add a page to the Word report doc: Title, chart, table, desc., limits

# DO NOT EDIT THIS, the parameters above will be used

# Notice we are not modifying Report, we are making a new copy of it
...
print(ReportNew, target = "Demo_Report.docx") # pushes all additions to the new file
...
We can also see what it's like to add an image after the text fields.  
...
image_file <- "C:/Users/libby/Documents/Portfolio/OfficerDemoWordOnly/ExamplePhoto.JPG"
...
ReportNew <- Report %>%
  ...
  body_add_par(value = "Assumptions", style = "heading 2") %>% 
  body_add_par(value = dataAssumptions, style = "Normal") %>%
  body_add_img(src = image_file, height = 4, width = 4, unit = "in")
